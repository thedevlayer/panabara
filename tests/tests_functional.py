...
from examples.models import MyStocks
...
class CRUDActionsTest(FunctionalTest):
    def setUp(self):
        super().setUp()
        self.MyStocks = MyStocks.objects.create(
    ...
    def test_create_object(self):
        ...
        # User clicks create MyStocks button
        self.browser.find_element_by_id('create-MyStocks-sync').click()

        # Create MyStocks modal opens
        ...
        mystock_type = form.find_element_by_id('id_mystock_type')
        mystock_type_select = Select(mystock_type)
        ...
        mystock_type_select.select_by_index(1)
        ...
        # Slice removes '\nx' since alert is dismissible and contains 'times' button
        success_msg = self.wait_for(class_name='alert').text[:-2]
        self.assertEqual(success_msg, 'Success: MyStocks was created.')

        # User sees created MyStocks in table
    ...
    def test_filter_object(self):
        ...
        # User clicks filter MyStocks button
        self.wait_for(element_id='filter-MyStocks').click()

        # Update filter modal opens
        modal = self.wait_for(element_id='modal')

        # User changes price and MyStocks type
        mystock_type = self.browser.find_element_by_id("id_type")
        mystock_type_select = Select(mystock_type)
        mystock_type_select.select_by_index(2)

        filter_btn = modal.find_element_by_class_name('submit-btn')
        filter_btn.click()

        # User is redirected to the index with a querystring with the filter
        self.wait_for(element_id='filter-MyStocks')
        redirect_url = self.browser.current_url
    ...
    def test_update_object(self):
        ...
        # User clicks update MyStocks button
        self.browser.find_element_by_class_name('update-MyStocks').click()

        # Update MyStocks modal opens
        modal = self.wait_for(element_id='modal')
        ...
        self.assertEqual(success_msg, 'Success: MyStocks was updated.')

        # User sees updated MyStocks in table
        table_entries = self.wait_for_table_rows()
    ...
    def test_read_object(self):
        ...
        # User clicks Read MyStocks button
        self.browser.find_element_by_class_name('read-MyStocks').click()

        # Read MyStocks modal opens
        modal = self.wait_for(element_id='modal')

        # User sees MyStocks content
    ...
    def test_delete_object(self):
        ...
        # User clicks Delete MyStocks button
        self.browser.find_element_by_class_name('delete-MyStocks').click()

        # Delete MyStocks modal opens
        modal = self.wait_for(element_id='modal')
        ...
        self.assertEqual(
            delete_text, 'Are you sure you want to delete MyStocks with title Life of John Doe?'
        )
        ...
        # Slice removes '\nx' since alert is dismissible and contains 'times' button
        success_msg = self.browser.find_element_by_class_name('alert').text[:-2]
        self.assertEqual(success_msg, 'Success: MyStocks was deleted.')

        # User sees 'No mystocks added yet.'
        no_mystocks = self.browser.find_element_by_class_name('no-mystocks')
        self.assertEqual(no_mystocks.text, 'No mystocks added yet.')

        # There is no mystocks in database anymore
        mystocks = MyStocks.objects.all()
        self.assertEqual(mystocks.count(), 0)
